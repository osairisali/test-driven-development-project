...
Scenario('searching movies', async ({ I }) => {
  ...
  assert.strictEqual(visibleLikedMovies, matchingMovies.length);

  for (let ind = 0; ind < matchingMovies.length; ind++) {
    const visibleTitle = await I.grabTextFrom(locate('.movie__title').at(ind + 1));
    assert.strictEqual(visibleTitle, matchingMovies[ind]);
  }
